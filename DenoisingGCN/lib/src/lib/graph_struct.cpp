...
GraphStruct::GraphStruct()
{
    ...
    edge_list.clear();
	out_noises = new LinkedTable< std::pair<int, int> >();
    in_noises = new LinkedTable< std::pair<int, int> >();
    noise_list.clear();
}
...
void GraphStruct::AddNoise(int idx, int x, int y)
{
    out_noises->AddEntry(x, std::pair<int, int>(idx, y));
    in_noises->AddEntry(y, std::pair<int, int>(idx, x));
	num_noises++;
    noise_list.push_back(std::make_pair(x, y));
    assert(num_noises == noise_list.size());
    assert(num_noises - 1 == (unsigned)idx);
}
...
void GraphStruct::Resize(unsigned _num_subgraph, unsigned _num_nodes)
{
	...
	out_noises->Resize(num_nodes);
	in_noises->Resize(num_nodes);
}
